<script setup lang="ts">
const emits = defineEmits(['submitKeyword'])

const enteredKeyword = defineModel<string>('enteredKeyword')

const submitHandler = () => {
  emits('submitKeyword', enteredKeyword.value)
}
</script>

<template>
  <form class="flex justify-center w-4/5" @submit.prevent="submitHandler">
    <label class="relative max-w-xs form-control">
      <div class="absolute -translate-y-1/2 label top-1/2 left-1">
        <span class="label-text text-slate-400">
          <svg
            xmlns="http://www.w3.org/2000/svg"
            fill="none"
            viewBox="0 0 24 24"
            stroke-width="1.5"
            stroke="currentColor"
            class="w-6 h-6"
          >
            <path
              stroke-linecap="round"
              stroke-linejoin="round"
              d="m21 21-5.197-5.197m0 0A7.5 7.5 0 1 0 5.196 5.196a7.5 7.5 0 0 0 10.607 10.607Z"
            />
          </svg>
        </span>
      </div>
      <input
        type="text"
        class="w-full max-w-xs pl-12 input input-bordered"
        v-model="enteredKeyword"
      />
    </label>
  </form>
</template>
